...
const myBot = ['ubcce77ee1567e766abdf17a66d9472f1'];
...
\n\n# http://line.me/ti/p/fcpea251  ^_^";
    ...
    poll(operation) {
		...
		if(operation.type == 16 && this.stateStatus.salam == 1){//join group
			...
			halo.text = "ข้ามาแล้วพร้อมเสียวกันรึยัง😘😘";
...
\n";
        ...
        if(txt == 'ดี') {
            ...
            this._sendMessage(seq, 'สวัสดีคับพูดเพราะจังขอเยที🙏')
        }
        if(txt == '555'){
            this._sendMessage(seq, '555ตลกไอ้สาสเดียวตบหัวจิ้มขี้เลย👣')
        }
        if(txt == '@บอทแทค') {
            this._sendMessage(seq, 'แทคหาฟ่องหรอสาสฟ่องตายไง👻')
        }
        if(txt == 'กำ') {
            this._sendMessage(seq, 'กำหีกำหำพ่อมึงอ่ะ👣(≧∇≦)/')
        }
        if(txt == 'ด่ามัน') {
            this._sendMessage(seq, 'ควยสาสหน้าหีลูกกรี่')
            this._sendMessage(seq, 'เยสแม่เดะมึงเจอกูสาส')
            this._sendMessage(seq, 'แม่มึงเปนกรี่พ่อเอาหัวดุดหีมึงลูกทะลุถุงยางสาส')
            this._sendMessage(seq, 'คิดว่าแน่หรอมาเจอกูได้ตัวๆกล้าป่าวสาส')
            this._sendMessage(seq, 'ไม่เจ๋งจริงอย่าห่าวเดียวมึงเจอสาส')
            this._sendMessage(seq, 'ฟ่องตายแม่งตายอีคายไอ้ควย')
            this._sendMessage(seq, 'กูได้หมุดอิควย')
        }
        if(txt == '.') {
            this._sendMessage(seq, 'พิมควยไรสาส')
        }
        if(txt == 'โหด') {
            this._sendMessage(seq, 'กูโหดยุแล้วสาส')
        }
        if(txt == 'เหี้ย') {
            this._sendMessage(seq, 'ก้มึงไงเหี้ยด่าตัวเองทำไมกูงง')
        }
        if(txt == 'งง') {
            this._sendMessage(seq, 'งงฟ่องเรียนมาน้อยรึไงสาส')
        }
        if(txt == 'บอท') {
            this._sendMessage(seq, 'เรียกทำส้นตีนไรเดียวบินแม่งทั่งกลุ่ม')
        }
        if(txt == 'มอนิ่ง') {
            this._sendMessage(seq, 'ตืนนานแล้วสาสมึงพึ่งตืนรึไงหน้าหี')
        }
        if(txt == 'ควย') {
            this._sendMessage(seq, 'ด่าตัวเองทำไมล่ะคับ')
        }
        if(txt == 'สาส') {
            this._sendMessage(seq, 'สาสที่ลุด')
        }
        if(txt == 'หน้าหี') {
            this._sendMessage(seq, 'อ่าวก้หน้ามึงไงด่าใครได้สาส')
        }
        if(txt == 'เออ') {
            this._sendMessage(seq, 'เออดีรู้เรื่อง')
        }
        if(txt == 'เงี่ยน') {
            this._sendMessage(seq, 'เงี่ยนก้มาเดะพี่จะปี้ให้ยับเลยยอกตรง')
        }
        if(txt == '55') {
            this._sendMessage(seq, 'ฟ่องตาย')
        }
        if(txt == '5555') {
            this._sendMessage(seq, 'ตลกหน้าหี')
        }
        if(txt == '-.-"') {
            this._sendMessage(seq, 'ด้านหน้า')
        }
        if(txt == 'คับ') {
            this._sendMessage(seq, 'คับก้ถอดดิ')
        }
        if(txt == 'บิน') {
            this._sendMessage(seq, 'มาดิสาสถ้าบินห้องที่กูยุได้อ่ะน่ะ')
        }
        if(txt == 'แมวเป้') {
            this._sendMessage(seq, 'โคตพ่อโคตแม่ควมหล่อ')
        }
		...
		if(txt == '0101' && lockt == 1) {//Jangan dicoba (gk ada efek)
            ...
            for (var i = 0; i < listMember.length; i++) {
                if(listMember[i].mid==param){
					let namanya = listMember[i].dn;
					seq.text = 'ดีสาสเข้ามาแล้วทำตัวดีๆ @'+namanya+', อย่ากวนส้นตีนไม่พอใจก้ออกห้องไป👣👣^_^';
					let midnya = listMember[i].mid;
